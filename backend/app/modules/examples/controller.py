...
from app.core.errors import ConfigurationError, DatabricksAPIError
...
async def serving(
    ...
    columns = list(dumped[0]) if dumped else []
    dataframe_split = {
        "index": list(range(len(dumped))),
        "columns": columns,
        "data": [[d[c] for c in columns] for d in dumped],
    }
...
class SqlQuery(BaseModel):
    ...
    table: str = Field(..., min_length=1, max_length=300, pattern=r"^[\w.$]+$")
...
def query_warehouse(
    ...
    if not warehouse_id:
        raise ConfigurationError("DATABRICKS_WAREHOUSE_ID not configured")

    ws = get_workspace_client(request)
    logger.info("Executing warehouse query on %s", warehouse_id)
    resp = ws.statement_execution.execute_statement(
        warehouse_id=warehouse_id,
        statement=f"SELECT * FROM {payload.table} LIMIT {payload.limit}",
        wait_timeout="30s",
    )
    if resp.status and resp.status.error:
        raise DatabricksAPIError(str(resp.status.error.message))
    columns = (
